...
for gqi_file in gqi_files[:80]:
    ...
    tractography_file = tractography_dir / f'{sub_id}.tt.gz'
    ...
    soft_link_path = train_dir / f'{sub_id}.tt.gz'
...
for gqi_file in gqi_files[80:90]:
    ...
    soft_link_path = val_dir / f'{sub_id}.tt.gz'
    if not soft_link_path.exists():
        soft_link_path.symlink_to(tractography_file)
...
for gqi_file in gqi_files[90:100]:
    ...
    soft_link_path = test_dir / f'{sub_id}.tt.gz'
    if not soft_link_path.exists():
        soft_link_path.symlink_to(tractography_file)
